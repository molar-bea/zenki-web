...
// Wrapping in reactive() automatically unwraps the nested refs!
...
const filteredSchedules = computed(() => {
  ...
  // Explicitly typing 's' as AppointmentScheduleView fixes the 'any' type error
...
.workspace-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; align-items: start; }
...
.panel__header { 
  ...
  margin-bottom: 1.5rem; border-bottom: 1px solid #E0E2DB; padding-bottom: 1rem;
}
.panel__header h2 { margin: 0; font-size: 1.25rem; color: #191716; }

.btn-flat { 
  background: #4F6367; color: #E0E2DB; border: none; padding: 0.6rem 1.2rem; 
...
.field-light span { font-size: 0.85rem; font-weight: 600; color: #4F6367; text-transform: uppercase; letter-spacing: 0.05em; }
.field-light input {
  width: 100%; padding: 0.8rem; border: 1px solid #B8D8D8; border-radius: 8px;
  background: #fcfcfc; color: #191716; font-size: 0.95rem; outline: none; transition: border-color 0.2s;
  font-family: inherit;
}
...
.priority-chip {
  flex: 1; padding: 0.6rem; border: 1px solid #B8D8D8; border-radius: 6px;
  background: transparent; color: #4F6367; font-weight: 600; cursor: pointer; transition: all 0.2s;
}
...
.filter-row { 
  ...
  background: #E0E2DB; padding: 0.35rem; border-radius: 8px; 
}
.filter-chip { 
  flex: 1; border: none; background: transparent; padding: 0.5rem; 
  border-radius: 6px; font-size: 0.8rem; font-weight: 600; color: #4F6367; 
...
.compact-card {
  display: flex; justify-content: space-between; align-items: center;
  padding: 0.75rem 1rem; border: 1px solid #E0E2DB; border-radius: 8px;
  background: #fafafa; transition: border-color 0.2s; gap: 1rem;
}
.compact-card--editing { border-color: #4F6367; background: #ffffff; box-shadow: 0 4px 12px rgba(122, 158, 159, 0.15); }

.compact-card__info { display: flex; flex-direction: column; gap: 0.35rem; }
.compact-date { margin: 0; font-size: 0.95rem; font-weight: 700; color: #191716; }
...
/* A soft, cool teal for Medical Exams */
.badge--medical { 
...
/* A warm, soft stone/fog for Document Drops */
.badge--document { 
...
.compact-card__controls { display: flex; align-items: center; gap: 1.25rem; }
.compact-stats { width: 120px; display: flex; flex-direction: column; gap: 0.35rem; }

.slot-meta { display: flex; justify-content: space-between; align-items: center; font-size: 0.75rem; color: rgba(25, 23, 22, 0.8); }
...
.inline-input { 
  width: 50px; padding: 0.1rem 0.3rem; border: 1px solid #7A9E9F; 
  border-radius: 4px; font-size: 0.8rem; outline: none; font-family: inherit; font-weight: 600;
}
...
.icon-btn {
  background: transparent; border: none; color: #B8D8D8; cursor: pointer;
  padding: 0.35rem; border-radius: 4px; transition: all 0.2s; display: grid; place-items: center;
}
...
@media (max-width: 600px) {
  .compact-card { flex-direction: column; align-items: flex-start; }
  .compact-card__controls { width: 100%; justify-content: space-between; }
}
</style>